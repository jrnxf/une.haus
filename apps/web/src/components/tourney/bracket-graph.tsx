    ...
    >
      ...
      <div className="flex items-center border-b">
        <button
          ...
          className={cn(
            ...
            canSelect && !player1IsBye && "hover:bg-muted/70 cursor-pointer",
            match.winner === 1 && "bg-muted/70 font-semibold",
        ...
        {canOpenTimer && (
          <button
            type="button"
            className="nodrag nopan text-muted-foreground hover:bg-muted/70 hover:text-foreground flex w-0 shrink-0 items-center justify-center self-stretch overflow-hidden transition-all duration-150 group-hover:w-8 group-hover:border-l"
            onClick={(e) => {
              e.stopPropagation()
      ...
      <button
        ...
        className={cn(
          ...
          canSelect && !player2IsBye && "hover:bg-muted/70 cursor-pointer",
          match.winner === 2 && "bg-muted/70 font-semibold",
...
function buildBracketGraph(
  ...
  for (let round = 2; round <= totalRounds; round++) {
    const matchesInThisRound = firstRoundMatchCount / 2 ** (round - 1)
  ...
  let boundsHeight =
    firstRoundMatchCount * (NODE_HEIGHT + BASE_VERTICAL_GAP) - BASE_VERTICAL_GAP
...
export function BracketContainer({
  ...
  return (
    <div ref={containerRef} className="relative min-h-0 flex-1">
      ...
      {code && (
        <span className="text-muted-foreground pointer-events-none absolute top-4 right-4 font-mono text-2xl tracking-widest">
          {code}
        </span>
      )}
      <Logo className="pointer-events-none absolute right-4 bottom-4 h-6 w-auto" />
    </div>
  )
}
