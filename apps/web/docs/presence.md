...
Entries older than 30 seconds (2x the poll interval) are pruned on every read/write, giving one missed poll of grace before a user drops off.
...
| `src/lib/presence/ops.server.ts`      | Server logic (heartbeat + query) |
| `src/lib/presence/fns.ts`             | Server function definition       |
| `src/lib/presence/hooks.ts`           | `useIsUserOnline` hook           |
...
- **Poll = heartbeat** — combining the two into one request keeps things simple. No separate heartbeat endpoint needed.
- **30s threshold** — 2x the 15s poll interval. One missed poll is tolerated; two consecutive misses marks the user as offline.
