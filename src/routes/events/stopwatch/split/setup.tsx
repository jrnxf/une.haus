...
export const Route = createFileRoute("/events/stopwatch/split/setup")({
...
function RouteComponent() {
  ...
  const [name1, setName1] = useState(search.name1 ?? "");
  const [name2, setName2] = useState(search.name2 ?? "");
  const [time, setTime] = useState(search.time);

  const handleStart = () => {
    navigate({
      to: "/events/stopwatch/split",
      search: {
        ...
        <div className="mx-auto w-full max-w-lg p-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle>Split Stopwatch</CardTitle>
              <CardDescription>
                Configure settings for both timers
              </CardDescription>
            </CardHeader>
