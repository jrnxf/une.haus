...
describe("calculatePoints", () => {
  ...
  it("calculates points for submissions only (3 points per submission)", () => {
  ...
  it("validates point constants", () => {
    ...
    expect(POINTS_PER_SUBMISSION).toBe(3);
  });
});
...
describe("compareRiders", () => {
  ...
  describe("tiebreaker: with submissions", () => {
    ...
    it("uses submission tiebreaker even when one rider has no sets", () => {
      ...
      // Both have 3 points from 1 submission
      const a = createRiderScore({
        userId: 1,
        points: 3,
        setsCount: 0,
    ...
    it("uses submission tiebreaker when only one rider has submissions", () => {
      // a: 4 points from 4 sets, no submissions
      // b: 4 points from 1 set + 1 submission
      const a = createRiderScore({
        userId: 1,
        points: 4,
        setsCount: 4,
...
describe("calculateRiderRankings", () => {
  ...
  it("ranks users by points (higher first)", () => {
    ...
    expect(rankings[0].user.id).toBe(200); // 6 points, rank 1
    expect(rankings[0].rank).toBe(1);
  ...
  describe("tiebreaker scenarios", () => {
    ...
    it("breaks tie with submissions by earlier last submission", () => {
      // User 100: 1 set + 1 submission = 4 points, last submission at 10:00
      // User 200: 1 set + 1 submission = 4 points, last submission at 12:00
      const sets = [
        createSet(1, 100, new Date("2024-01-01T08:00:00Z"), [
    ...
    it("user with submission beats user with only sets when tied on points", () => {
      // User 100: 4 sets = 4 points, last set at 10:00
      // User 200: 1 set + 1 submission = 4 points, last submission at 12:00
      const sets = [
        createSet(1, 100, new Date("2024-01-01T08:00:00Z")),
        createSet(2, 100, new Date("2024-01-01T09:00:00Z")),
        createSet(3, 100, new Date("2024-01-01T10:00:00Z")),
        createSet(4, 100, new Date("2024-01-01T11:00:00Z")),
        createSet(5, 200, new Date("2024-01-01T07:00:00Z"), [
          createSubmission(1, 200, new Date("2024-01-01T12:00:00Z")),
        ]),
      ];
      const rankings = calculateRiderRankings(sets);
  ...
  describe("complex scenarios", () => {
    it("handles multiple users with mixed sets and submissions", () => {
      ...
      // User 200: 1 set + 2 submissions = 7 points
      // User 300: 3 submissions = 9 points
      const sets = [
        createSet(1, 100, new Date("2024-01-01"), [
          createSubmission(1, 200, new Date("2024-01-02")),
      ...
      expect(rankings[0].user.id).toBe(300); // 9 points
      expect(rankings[0].points).toBe(9);
      expect(rankings[0].rank).toBe(1);

      expect(rankings[1].user.id).toBe(200); // 7 points
      expect(rankings[1].points).toBe(7);
      expect(rankings[1].rank).toBe(2);
    ...
    it("handles three-way tie with submissions", () => {
      // All three users have 1 set + 1 submission = 4 points each
  ...
  describe("edge cases", () => {
    ...
    it("user appears in multiple sets' submissions", () => {
      ...
      expect(submitter?.points).toBe(9);
    });
  });
});
