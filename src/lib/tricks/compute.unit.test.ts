...
import type { Trick, TrickModifiers } from "./types";
...
describe("describeModifierDiff", () => {
  ...
  it("describes boolean toggle removals", () => {
    expect(
      describeModifierDiff(baseMods({ fakie: true }), baseMods()),
    ).toBe("remove fakie");
  });
...
describe("modifierDirection", () => {
  ...
  it("counts tire none→something as adds", () => {
    expect(
      modifierDirection(baseMods(), baseMods({ tire: "to tire" })),
    ).toBe("adds");
  });

  it("counts tire something→none as removes", () => {
    expect(
      modifierDirection(baseMods({ tire: "from tire" }), baseMods()),
    ).toBe("removes");
  });
  ...
  it("counts boolean toggle true as adds", () => {
    ...
    expect(modifierDirection(baseMods(), baseMods({ late: true }))).toBe(
      "adds",
    );
  });
...
describe("computeAllNeighbors", () => {
  ...
  it("labels compound-component neighbors correctly", () => {
    ...
    const compoundNeighbor = tricks[1]!.neighbors.find(
      (n) => n.id === "flip",
    );
    expect(compoundNeighbor?.label).toBe("component");
    expect(compoundNeighbor?.direction).toBe("removes");

    const simpleNeighbor = tricks[0]!.neighbors.find(
      (n) => n.id === "combo",
    );
  ...
  it("connects compound siblings sharing a component", () => {
    const component = makeTrick({ id: "flip", modifiers: baseMods({ flips: 1 }) });
    const combo1 = makeTrick({
      id: "combo1",
      isCompound: true,
      compositions: [
        { componentId: "flip", componentName: "Flip", position: 0, catchType: null },
        { componentId: "spin", componentName: "Spin", position: 1, catchType: null },
      ],
    });
    const combo2 = makeTrick({
      ...
      compositions: [
        { componentId: "flip", componentName: "Flip", position: 0, catchType: null },
        { componentId: "roll", componentName: "Roll", position: 1, catchType: null },
      ],
    });
...
describe("buildIndexes", () => {
  ...
  it("collects prefix tricks", () => {
    const tricks = [
      makeTrick({ id: "a", isPrefix: true, depth: 0 }),
      makeTrick({ id: "b", isPrefix: false, depth: 0 }),
      makeTrick({ id: "c", isPrefix: true, depth: 0 }),
    ];
    const result = buildIndexes(tricks);
    expect(result.prefixes).toHaveLength(2);
    expect(result.prefixes.map((t) => t.id)).toEqual(["a", "c"]);
  });
