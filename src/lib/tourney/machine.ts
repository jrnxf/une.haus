...
function selectWinnerContext(
  ...
  const sortedMatches = [...bracket].toSorted((a, b) => {
        ...
        "prelim.disqualifyRider": {
          actions: assign({
            prelimStatuses: ({ context, event }) => {
              ...
              return {
                ...context.prelimStatuses,
                [e.riderIndex]: "dq" as const,
              }
            },
          }),
        },
...
type TournamentStateValue =
  ...
  | {
      prelims:
        | "idle"
        | { riderActive: "timerIdle" | "timerRunning" | "timerPaused" }
    }
  | "ranking"
