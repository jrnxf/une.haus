...
const { tricks, trickElements, trickElementAssignments, trickRelationships } =
  schema
...
const ALL_ELEMENTS: { slug: string; name: string }[] = [
  { slug: "spin", name: "spin" },
  { slug: "flip", name: "flip" },
  { slug: "wrap", name: "wrap" },
  { slug: "twist", name: "twist" },
  { slug: "roll", name: "roll" },
  { slug: "grind", name: "grind" },
  { slug: "varial", name: "varial" },
  ...
  { slug: "ride", name: "ride" },
  { slug: "handstepover", name: "handstepover" },
  { slug: "stepover", name: "stepover" },
  { slug: "plant", name: "plant" },
  { slug: "combo", name: "combo" },
  { slug: "position", name: "position" },
  { slug: "pirouette", name: "pirouette" },
  { slug: "footjam", name: "footjam" },
  { slug: "crownjam", name: "crownjam" },
  { slug: "toetwist", name: "toetwist" },
  { slug: "pedal-push", name: "pedal push" },
  { slug: "bc-wheel", name: "bc wheel" },
...
async function main() {
  ...
  const existingTricks = await db
    ...
    .from(tricks)
  const existingTrickSlugs = new Map(existingTricks.map((t) => [t.slug, t.id]))
    ...
    if (existingId) {
      ...
      await db
        ...
        .set({
          ...
          alternateNames:
            row.alternateNames.length > 0 ? row.alternateNames : undefined,
  ...
  for (const row of csvRows) {
    ...
    const assignments = categorySlugs
      .map((catSlug) => {
        ...
        if (!elementId) {
          console.warn(
            `  Warning: Unknown category "${catSlug}" for trick "${row.name}"`,
          )
          return null
        }
  ...
  for (const row of csvRows) {
    ...
    if (row.optionalPrerequisiteName) {
      ...
      for (const optName of optPrereqNames) {
        const optSlug = nameToSlug.get(optName.toLowerCase()) ?? toSlug(optName)
